...
# }}}
# {{{ compare C to Harrel's C uncensored data
...
stopifnot(harrelC1[["C Index"]]==Cpec$AppCindex[["Cox X1"]])
stopifnot(harrelC1[["Relevant Pairs"]]==Cpec$Pairs[["Cox X1"]])
stopifnot(harrelC1[["Concordant"]]==Cpec$Concordant[["Cox X1"]])
stopifnot(harrelC2[["C Index"]]==Cpec$AppCindex[["Cox X2"]])
stopifnot(harrelC2[["Relevant Pairs"]]==Cpec$Pairs[["Cox X2"]])
stopifnot(harrelC2[["Concordant"]]==Cpec$Concordant[["Cox X2"]])
stopifnot(harrelC12[["C Index"]]==Cpec$AppCindex[["Cox X1+X2"]])
stopifnot(harrelC12[["Relevant Pairs"]]==Cpec$Pairs[["Cox X1+X2"]])
stopifnot(harrelC12[["Concordant"]]==Cpec$Concordant[["Cox X1+X2"]])
...
harrelC3 <- rcorr.cens(dummy$x,with(dummy,Surv(time,status)),outx=FALSE)
harrelC3a <- rcorr.cens(dummy$x,with(dummy,Surv(time,status)),outx=TRUE)
...
stopifnot(C3$AppCindex[["x"]]==harrelC3["C Index"])
stopifnot(C3a$AppCindex[["x"]]==harrelC3a["C Index"])
...
harrelC4a <- rcorr.cens(dummy2$x,with(dummy2,Surv(time,status)),outx=TRUE)
...
stopifnot(C4a$AppCindex[["x"]]!=harrelC4a["C Index"])
stopifnot(C4b$AppCindex[["x"]]==harrelC4a["C Index"])
message("DONE:test effect of ties in outcome on c-index ")
# }}}
# {{{ compare C to Harrel's C with censored data
#
# Harrels index ignores pairs where Y[i]==Y[j]
...
harrelC5 <- rcorr.cens(dummy3$x,with(dummy3,Surv(time,status)),outx=FALSE)
