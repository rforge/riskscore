...
perr.link <- pec(list(AbsRisk=arr.fit,CauseSpecCox=cox.fit,FineGray=fg.fit,LogisticRisk=lrr.fit),formula=Hist(time,status)~sex+epicel+ulcer+age+logthick,cens.model="cox",data=Melanoma,maxtime=2500)

perr.link1 <- pec(list(LogisticRisk=lrr.fit),formula=Hist(time,status)~1,times=1499,exact=FALSE,start=NULL,cause=1,cens.model="marginal",data=Melanoma[1:10,],maxtime=2500,reference=FALSE)
perr.link2 <- pec(list(LogisticRisk=lrr.fit),formula=Hist(time,status)~1,times=c(1499,1500),exact=FALSE,start=NULL,cause=1,cens.model="marginal",data=Melanoma[1:10,],maxtime=2500,reference=FALSE)
...
f2  <- CSC(Hist(time,status)~sex,cause=2,data=pbc)
