  ...
  if (splitMethod$internal.name %in% c("Boot632plus")){
    if (noinf.permute==FALSE){
      NoInfErr <- lapply(1:NF,function(f){
        ...
        extraArgs <- model.args[[f]]
        if (predictHandlerFun=="predictEventProb")
          .C("pseudoPec_noinfCR",pseudoPec=double(NT),as.double(YY),as.double(event),as.double(times),as.double(pred),as.integer(N),as.integer(NT),NAOK=TRUE,PACKAGE="pec")$pseudoPec
        else
          .C("pseudoPec_noinf",pseudoPec=double(NT),as.double(YY),as.double(times),as.double(pred),as.integer(N),as.integer(NT),NAOK=TRUE,PACKAGE="pec")$pseudoPec
      })
      ...
      NoInfErrList <- lapply(1:B,function(b){
        ...
        noinfPredErr <- lapply(1:NF,function(f){
          ...
          if (predictHandlerFun=="predictEventProb")
            .C("pseudoPecCR",pseudoPec=double(NT),as.double(YY),as.double(event),as.double(times),as.double(pred.b),as.integer(N),as.integer(NT),NAOK=TRUE,PACKAGE="pec")$pseudoPec
          else
            .C("pseudoPec",pseudoPec=double(NT),as.double(YY),as.double(times),as.double(pred.b),as.integer(N),as.integer(NT),NAOK=TRUE,PACKAGE="pec")$pseudoPec
        })
        noinfPredErr
      })
