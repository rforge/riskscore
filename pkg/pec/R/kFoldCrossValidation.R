kFoldCrossValidation <- function(object,
  ...
  CrossValErrMat <- lapply(1:B,function(b,extraArgs=extraArgs){
    ...
    pec <- lapply(1:NF,function(f){
      ...
      if (predictHandlerFun=="predictEventProb")
        innerCrossValErr <- .C("pecCR",pec=double(NT),as.double(Y),as.double(status),as.double(event),as.double(times),as.double(pred.b),as.double(ipcw$IPCW.times),as.double(ipcw$IPCW.subjectTimes),as.integer(N),as.integer(NT),as.integer(ipcw$dim),NAOK=TRUE,PACKAGE="pec")$pec
      else
        innerCrossValErr <- .C("pec",pec=double(NT),as.double(Y),as.double(status),as.double(times),as.double(pred.b),as.double(ipcw$IPCW.times),as.double(ipcw$IPCW.subjectTimes),as.integer(N),as.integer(NT),as.integer(ipcw$dim),NAOK=TRUE,PACKAGE="pec")$pec
      innerCrossValErr
    })
